...
export default function App() {
  ...
  useEffect(() => {
    ...
    loadResults(session.user.id).then(setResults).finally(() => setLoadingResults(false))
  ...
  if (!authReady) return (
    <div style={{ minHeight:'100vh', background:'var(--bg)', display:'flex', alignItems:'center', justifyContent:'center', flexDirection:'column', gap:12 }}>
      <div style={{ width:40, height:40, borderRadius:10, background:'linear-gradient(135deg,var(--teal),var(--blue))', display:'flex', alignItems:'center', justifyContent:'center', fontWeight:700, fontSize:18, color:'#000' }}>E</div>
  ...
  return (
    <div style={{ background:'var(--bg)', minHeight:'100vh', color:'var(--text)' }}>
      <Nav page={page} setPage={setPage} dark={dark} setDark={setDark} user={session.user} profile={profile}/>
      ...
      {page === 'My Plan'       && (
        <div className="app-container">
          <h2 style={{ fontSize:22, fontWeight:700, color:'var(--text)', marginBottom:4 }}>My Learning Plan</h2>
          <p style={{ color:'var(--textM)', fontSize:13, marginBottom:22 }}>Choose your study period and get a personalised daily schedule with email reminders.</p>
