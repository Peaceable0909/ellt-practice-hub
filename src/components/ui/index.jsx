...
export function Chip({ text, color }) {
  return (
    <span style={{ fontSize: 11, fontWeight: 800, padding: '3px 9px', borderRadius: 100, background: `${color}22`, color, border: `2px solid ${color}44`, letterSpacing: '0.3px', whiteSpace: 'nowrap', display: 'inline-block', textTransform: 'uppercase' }}>
      {text}
    </span>
  )
}
...
export function FeedbackBlock({ feedback, accentColor }) {
  ...
  return (
    <div style={{ marginTop: 14, background: 'var(--bg3)', border: `2px solid ${accentColor}44`, borderRadius: 16, padding: 16, borderBottom: `4px solid ${accentColor}66` }}>
      ...
      {lines.map((line, i) => {
        ...
        return (
          <div key={i} style={{
            ...
            lineHeight: 1.6, textTransform: isHeader ? 'uppercase' : 'none',
          }}>
